...
export interface PeginModule {
  getPeginAddress: (pubKey: string, fedPegScript: string, network: number, isDynaFed: boolean, contract: string) => string
}
...
loadModule().then((mod: PeginModule) => {
  const { publicKey } = ECPair.makeRandom({ network: networks.regtest })
  const r = mod.getPeginAddress(
